...
def create_recipe_form(
    ...
    try:
        ...
        if len(instruction_list) == 0:
            raise ValueError("At least one instruction step required")
        
        recipe_data = RecipeCreate(
            title=title,
            description=description,
            cuisine=cuisine,  # New field
            ingredients=ingredient_list,
            instructions=instruction_list,  # Changed to list
            tags=tag_list
        )
